...
async function restablecerFechaInicio() {
    ...
    let confirmar = confirm(
        "⚠️ ¿Seguro que deseas restablecer la fecha de inicio?\n\nEsto permitirá configurar una nueva fecha para las asistencias."
    );
...
async function buscarAlumnosRealTime(inputId, listaId) {
    ...
    let filtrados = alumnos.filter(a => 
        a.nombreCompleto.toLowerCase().includes(buscar) ||
        (a.nombre && a.nombre.toLowerCase().includes(buscar)) ||
        (a.grupo && a.grupo.toLowerCase().includes(buscar)) ||
        (a.carrera && a.carrera.toLowerCase().includes(buscar))
    );
    ...
    if (filtrados.length === 0) {
        lista.innerHTML = `
            <div style="grid-column:1/-1; text-align:center; padding:50px; color:#666; background:rgba(248,249,250,0.8); border-radius:20px;">
                ...
                <p>Grupos disponibles: ${[...new Set(alumnos.map(a=>a.grupo))].join(', ')}</p>
            </div>
        `;
        return;
    }
    ...
    filtrados.slice(0, 12).forEach((alumno, index) => {
        ...
        div.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                ...
                <span style="background:linear-gradient(135deg,#4facfe,#00f2fe); color:white; padding:8px 15px; border-radius:25px; font-size:14px; font-weight:600;">#${index+1}</span>
            </div>
            <p style="margin:8px 0; color:#666; font-size:15px;">
                📚 <strong>${alumno.curso}° ${alumno.paralelo}</strong><br>
                🎓 <strong>${alumno.especialidad || 'Sin especialidad'}</strong>
            </p>
            <div style="margin-top:20px;">
                <button class="btn-calificar" onclick="abrirModalCalificar('${alumno.id}', '${alumno.nombre}')" 
...
function abrirModalCalificar(usuarioAlumno, nombreAlumno) {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <select id="materiaCalificar" style="width:100%; padding:15px; margin-bottom:25px; border-radius:12px; border:3px solid #e1e5e9; font-size:16px;">
            ...
            <label style="font-weight:700; display:block; margin-bottom:18px; color:#2c3e50;">⭐ Selecciona la conducta:</label>
            ...
            <div id="rangoSeleccionadoDocente" style="display:none; padding:20px; background:linear-gradient(135deg,#d4edda,#c3e6cb); border-radius:15px; margin-bottom:25px; text-align:center; font-weight:700; border:3px solid #28a745; color:#155724;"></div>
            
            <label style="font-weight:700; display:block; margin-bottom:12px; color:#2c3e50;">💬 Observaciones (opcional):</label>
            <textarea id="obsCalificar" style="width:100%; height:130px; padding:18px; border-radius:12px; border:3px solid #e1e5e9; font-size:16px; font-family:inherit; resize:vertical; margin-bottom:30px;" placeholder="Describe el comportamiento específico del alumno... (máx. 200 caracteres)"></textarea>
...
async function cargarConductaAlumno() {
    ...
    try {
        ...
        const respuesta = await fetch(
            `http://localhost:3000/conductas/${user.nombreCompleto}`
        );
