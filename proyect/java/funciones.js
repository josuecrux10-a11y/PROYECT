...
document.getElementById("registro").style.display = "none";
            ...
            if(user.rol === "alumno"){
                ...
                document.getElementById("panelSoporte")
                    .style.display = "block";
...
async function asignarMateriaDocente(uid) {
    const select = document.getElementById("materia_" + uid);

    // CORRECCIÓN 1: Obtener el ID (value) para la base de datos
    const materiaId = select.value; 
    const materiaNombre = select.options[select.selectedIndex].text;

    // Validación corregida
    if (!materiaId || materiaId === "" || materiaNombre === "📚 Seleccionar materia") {
        alert("Seleccione una materia");
        return;
    }

    try {
        const response = await fetch("http://localhost:3000/asignar-materia", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                docente_id: uid,
                materia_id: materiaId // <-- Ahora envías el ID real (ej: "9")
            })
        });

        // CORRECCIÓN 2: Forzar el error si el backend responde con fallas (400, 500, etc.)
        if (!response.ok) {
...
async function administrarUsuario(id){
    ...
    try{
        ...
        document.getElementById("adminRol").innerText =
            usuario.rol;
        ...
        if(usuario.rol === "alumno"){
            ...
            html = `
                ...
                <button
                    class="btnGuardar"
                    onclick="habilitarEdicionPerfil(${usuario.id})">
